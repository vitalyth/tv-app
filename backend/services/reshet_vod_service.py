...
RESHET_VOD_DB_PATH = get_vod_db_path("RESHET_VOD_DB_PATH")
RESHET_VOD_RETRIES = int(
    get_vod_env("RESHET_VOD_RETRIES", "VOD_RETRIES", "KAN_VOD_RETRIES", default="3")
)
RESHET_VOD_RETRY_DELAY_SECONDS = float(
    get_vod_env(
        "RESHET_VOD_RETRY_DELAY_SECONDS",
...
RESHET_VOD_STREAM_BATCH_SIZE = int(
    get_vod_env("RESHET_VOD_STREAM_BATCH_SIZE", "VOD_STREAM_BATCH_SIZE", "KAN_VOD_STREAM_BATCH_SIZE", default="20")
)
...
def _connect() -> sqlite3.Connection:
    db_path = prepare_vod_db_path(RESHET_VOD_DB_PATH)
    con = connect_vod_db(db_path)
    try:
        ensure_vod_provider_schema(db_path, "reshet", lambda: _init_db(con))
        return con
    except Exception:
        con.close()
        raise
...
def _init_db(con: sqlite3.Connection) -> None:
    ...
    _add_column_if_missing(con, "reshet_programs", "last_incremental_scan_at", "TEXT")
    ...
    ensure_unified_schema(con, providers=("reshet",))
    con.commit()
...
def _scan_program(
    ...
    mark_vod_program_detail_scan(con, "reshet_programs", program_id)
    con.commit()
    ...
    try:
        ...
        if with_details:
            selected_programs, scan_summary = select_vod_programs_for_detail_scan(
                con,
                programs,
                program_table="reshet_programs",
                episode_table="reshet_episodes",
            ...
            if verbose:
                print(f"Reshet scan summary: {scan_summary}", flush=True)
...
def get_reshet_vod_series(
    ...
    try:
        ...
        all_rows = con.execute(
            ...
            LEFT JOIN reshet_episodes e ON e.program_id = p.id
            LEFT JOIN ({vod_episode_activity_subquery("reshet")}) ve ON ve.program_id = p.id
            {where_sql}
            GROUP BY p.id
            ORDER BY
                CASE WHEN COUNT(DISTINCT e.id) > 0 THEN 0 ELSE 1 END,
...
def get_reshet_vod_recent_episodes(limit: int = 10) -> list[dict]:
    con = _connect()
    try:
        episode_columns = _table_columns(con, "reshet_episodes")
