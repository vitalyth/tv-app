...
def _fetch_keshet_vod_recent_items(limit: int) -> list[dict]:
    recent_items: list[dict] = []

    for index, episode in enumerate(get_keshet_vod_recent_episodes(limit)):
        episode_id = str(episode.get("id") or "")
        if not episode_id:
            continue
        ...
        recent_item = {
            ...
            "seasonName": season_title,
            "channelName": "קשת VOD",
            "channelImage": normalize_vod_image("mako.png"),
            "episodeName": title,
            "episodeDescription": description,
            "episodeImage": episode_image,
            "isFolder": False,
...
def _fetch_reshet_vod_recent_items(limit: int) -> list[dict]:
    recent_items: list[dict] = []

    for index, episode in enumerate(get_reshet_vod_recent_episodes(limit)):
        episode_id = str(episode.get("id") or "")
        if not episode_id:
            continue
        ...
        recent_item = {
            ...
            "seasonName": season_title,
            "channelName": "רשת VOD",
            "channelImage": normalize_vod_image("13.jpg"),
            "episodeName": title,
            "episodeDescription": description,
            "episodeImage": episode_image,
            "isFolder": False,
...
def _fetch_c14_vod_recent_items(limit: int) -> list[dict]:
    recent_items: list[dict] = []

    for index, episode in enumerate(get_c14_vod_recent_episodes(limit)):
        episode_id = str(episode.get("id") or "")
        if not episode_id:
            continue
        ...
        recent_item = {
            ...
            "channelImage": normalize_vod_image("14tv.png"),
            "episodeName": title,
            "episodeDescription": description,
            "episodeImage": episode_image,
            "isFolder": False,
...
def _fetch_direct_vod_recent_items(channel: dict, limit: int, use_cache: bool) -> list[dict]:
    ...
    if channel["id"] == "vod_keshet12":
        return _fetch_keshet_vod_recent_items(limit)

    if channel["id"] == "vod_reshet13":
        return _fetch_reshet_vod_recent_items(limit)
