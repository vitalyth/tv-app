...
const getSeasonTitle = (seasonId: string, series: VodProviderSeriesDetails) => {
  const season = series.seasons.find((item) => item.season_id === seasonId);
  if (season?.title) return season.title;
  return "פרקים";
};

const getEpisodeTitle = (episode: VodProviderEpisode) => {
  return episode.episodeName || episode.title || `פרק ${episode.id}`;
};

const getEpisodeImage = (series: VodProviderSeriesDetails, episode: VodProviderEpisode) => {
  return episode.episodeImage || episode.image || series.image || "/ch/vod.jpg";
};

const getEpisodeMetaText = (episode: VodProviderEpisode) => {
  return episode.published || "";
};

const episodeToVodItem = (
  series: VodProviderSeriesDetails,
  ...
  return {
    ...
    channelImage: config.channelLogo || series.image || "",
    episodeImage: image,
    isFolder: false,
    isPlayable: true,
  };
...
const episodeToVodStack = (
  series: VodProviderSeriesDetails,
  episode: VodProviderEpisode,
  config: VodProviderDetailsConfig,
): VodNode[] => {
  const image = config.channelLogo || series.image || getEpisodeImage(series, episode);
  ...
  return [
    ...
    },
    {
      ...
      module: config.module,
      mode: 0,
      url: `/${config.providerPath}/${encodeURIComponent(series.id)}`,
      logo: image,
      moreData: "",
...
const saveRecentVodItem = (
  ...
  if (typeof window === "undefined") return;
  try {
    const item = episodeToVodItem(series, episode, config);
    const stack = episodeToVodStack(series, episode, config);
    const existing = loadRecentVodItems().filter((saved) => saved.item.id !== item.id || saved.item.module !== item.module);
...
const episodeToChannel = (
  ...
  return {
    ...
    moreData: "",
    playerLogo: config.channelLogo || image,
...
export type VodProviderDetailsConfig = {
  channelLogo: string;
  channelName: string;
  module: string;
  providerPath: string;
...
export function VodProviderDetailsPage({ config }: { config: VodProviderDetailsConfig }) {
  ...
  const {
    ...
    error,
    mutate,
  } = useSWR([config.providerPath, programId], () => config.service.getSeriesDetails(programId), {
    revalidateOnFocus: false,
    dedupingInterval: 30000,
  });

  const episodesBySeason = useMemo(() => {
    if (!series) return {} as Record<string, VodProviderEpisode[]>;

    return series.episodes.reduce<Record<string, VodProviderEpisode[]>>((acc, episode) => {
  ...
  useEffect(() => {
    if (
      !series ||
      currentChannel?.module !== config.module ||
  ...
  }, [config.module, currentChannel, episodesBySeason, programId, series]);

  const handleBack = () => {
    router.push(`/${config.providerPath}`);
  };

  const playEpisode = useCallback((episode: VodProviderEpisode) => {
    if (!series) return;

    saveRecentVodItem(series, episode, config);
    play(episodeToChannel(series, episode, config));
  }, [config, play, series]);
  ...
  const refresh = async () => {
    ...
    try {
      await mutate(() => config.service.getSeriesDetails(programId, true), { revalidate: false });
    } finally {
  ...
  const playingEpisodeId = currentChannel?.module === config.module ? currentChannel.id : null;
  ...
  const detailActions: VodDetailAction[] = [
    {
      key: "back",
      label: `חזרה ל${config.providerTitle}`,
